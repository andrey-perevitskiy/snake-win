...
menu_cb_create_con (void * data)
{
    ...
    if (LOBYTE(ctx->wsaData.wVersion) != 2
            || HIBYTE(ctx->wsaData.wVersion) != 2) {
...
menu_cb_connect_to_con (void * data)
{
    ...
    fstat = connect(ctx->friendFd, (LPSOCKADDR) &ctx->comAddr,
        sizeof(ctx->comAddr));
    if (fstat == SOCKET_ERROR) {
...
inet_pton (int inet, const char * src, void * dst)
{
    ...
    if (WSAStringToAddress(src_copy, inet, NULL, (LPSOCKADDR) &storage, &size)
            == 0)
        switch (inet) {
        case PF_INET:
            *(struct in_addr *) dst
...
main (void)
{
    ...
    while (!ctx.isExit) {
        ...
        if (ctx.conOpt != CON_SINGLE) {
            ...
            score_draw(SCORE_POS_X, SCORE_POS_Y + WIN_SIZE_Y + 2,
                ctx.friend.s);
