...
cursor_erase (const struct menu * m)
{
    ...
    SetConsoleCursorPosition(h,
        (COORD) {getmposx(m) - 2 + item->indent, getmposy(m) + m->offset});
    printf("  ");

    SetConsoleCursorPosition(h,
        (COORD) {getmposx(m) + strlen(item->name) + item->indent,
        getmposy(m) + m->offset});
    printf("  ");
}
...
menu_draw (const struct menu * m)
{
    for (int i = 0; i < list_count(m->items); i++) {
        ...
        SetConsoleCursorPosition(h, (COORD) {getmposx(m) + item->indent,
            getmposy(m) + i});
        printf(item->name);
    }
}
